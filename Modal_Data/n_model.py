...
# from tensorflow.python.keras.layers import Embedding, LSTM a
...
class sequence_CNN(Layer):
    ...
    def __init__(self, units, kernal):
        ...
        self.units = units
        self.kernal = kernal
        self.conv_layer = Conv2D(self.units, self.kernal, padding='same')

    def call(self, inputs):
        timesteps = int(inputs.shape.as_list()[1])
        result = self.conv_layer(inputs[:, 0])
        result = tf.expand_dims(result, 1)
        for i in range(1, timesteps):
            conv_1 = self.conv_layer(inputs[:, i])
            conv_1 = tf.expand_dims(conv_1, 1)
            result = tf.concat([result, conv_1], 1)
        return result
...
def cbam_module(inputs):
    ...
    savg_pool = tf.reduce_mean(inputs1, axis=4, keepdims=True)  # keep_dims:是否降维
    smax_pool = tf.reduce_max(inputs, axis=4, keepdims=True)
...
# from keras.layers import Dense, AveragePooling2D, MaxPooling2D, Conv2D

# def cbam_module(inputs):
#     # channel attention
#     avgpool_channel = AveragePooling2D()(inputs)
#     maxpool_channel = MaxPooling2D()(inputs)
#     avg_pool = Dense(16, activation='relu')(avgpool_channel)
#     max_pool = Dense(16, activation='relu')(maxpool_channel)
#     avg_pool = Dense(32, activation='relu')(avg_pool)
#     max_pool = Dense(32, activation='relu')(max_pool)
...
#     inputs1 = Dense(32, activation='relu')(inputs)
#     inputs1 = Dense(64, activation='relu')(inputs1)
#     savg_pool = tf.reduce_mean(inputs1, axis=[1, 2], keepdims=True)
#     smax_pool = tf.reduce_max(inputs, axis=[1, 2], keepdims=True)
#     channel_w_pool = tf.concat([savg_pool, smax_pool], axis=3)

#     spatial_attention = Conv2D(filters=1, kernel_size=1)(channel_w_pool)
#     spatial_attention = tf.nn.sigmoid(spatial_attention)
#     spatial_attention_module = tf.multiply(inputs, spatial_attention)
...
class CBAMModule(Layer):
    def __init__(self, **kwargs):
        ...
        self.avg_pool = Dense(16, activation='relu')
        self.max_pool = Dense(16, activation='relu')
        self.fc_2 = Dense(32, activation='relu')
        self.channel_w_pool = Dense(64, activation='relu')
        self.spatial_attention_cnn = Conv3D(1, (1, 1, 1), activation='relu')

        # Initialize spatial attention layers
        self.spatial_dense1 = Dense(32, activation='relu')
        self.spatial_dense2 = Dense(64, activation='relu')

    def call(self, inputs, **kwargs):
        # channel attention
        avgpool_channel = tf.reduce_mean(inputs, axis=[1, 2, 3], keepdims=True)
        maxpool_channel = tf.reduce_max(inputs, axis=[1, 2, 3], keepdims=True)
        avg_pool = self.avg_pool(avgpool_channel)
        max_pool = self.max_pool(maxpool_channel)
        avg_pool = self.fc_2(avg_pool)
        max_pool = self.fc_2(max_pool)
        fc_2_output = tf.nn.sigmoid(avg_pool + max_pool)
        channel_attention_module = inputs * fc_2_output

       # Spatial attention
        inputs1 = self.spatial_dense1(inputs)
        inputs1 = self.spatial_dense2(inputs1)
        savg_pool = tf.reduce_mean(inputs1, axis=[1, 2, 3], keepdims=True)
        smax_pool = tf.reduce_max(inputs, axis=[1, 2, 3], keepdims=True)
        channel_w_pool = self.channel_w_pool(tf.concat([savg_pool, smax_pool], axis=4))

        spatial_attention = self.spatial_attention_cnn(channel_w_pool)
        spatial_attention = tf.nn.sigmoid(spatial_attention)
        spatial_attention_module = inputs * spatial_attention

        return channel_attention_module + spatial_attention_module
...
def trainModel(output_dim=25):
    ...
    conv1_1 = PReLU(alpha_initializer='zeros', alpha_regularizer=None, alpha_constraint=None, shared_axes=None)(conv1_1)
    conv1_2 = Conv3D(32, kernel_size=(3, 3, 3), strides=(1, 1, 1), padding='same', name='conv1_2')(conv1_1)  # 默认设置 chnnels在最后一维
    conv1_2 = PReLU(alpha_initializer='zeros', alpha_regularizer=None, alpha_constraint=None, shared_axes=None)(conv1_2)
    ...
    conv2_1 = PReLU(alpha_initializer='zeros', alpha_regularizer=None, alpha_constraint=None, shared_axes=None)(conv2_1)

    conv2_2 = sequence_CNN(32, kernal=3)(conv2_1)
    conv2_2 = PReLU(alpha_initializer='zeros', alpha_regularizer=None, alpha_constraint=None, shared_axes=None)(conv2_2)
    ...
    cbam_module1 = CBAMModule()
    attention_mulca = cbam_module1(conv2_2)


    # c_addc = concatenate([conv1_2, attention_mulca])
    c_addc = concatenate([conv1_2, attention_mulca])
    conv3_1 = sequence_CNN(1, kernal=3)(c_addc)
    conv3_1 = PReLU(alpha_initializer='zeros', alpha_regularizer=None, alpha_constraint=None, shared_axes=None)(conv3_1)
    ...
    dense_d = Dense(256, activation='linear')(dense)

    output = Dense(output_dim, activation='tanh')(dense_d)  # linear;tanh;sigmoid
...
for i in range(0, rows):
    for j in range(0, cols):
        ...
        # The format of the input space portion of the dataset is as follows:
        ...
        # The format of the input time portion of the dataset is as follows:
